/**
 * vel_gui_ctrl_node.cpp
 * 功能：创建一个带 Qt5 图形界面的 ROS2 节点，用户可以在界面上输入线速度和角速度（度/s），
 *       点击"启动"按钮后，持续发布速度指令控制小车运动；
 *       点击"停止"按钮后，发送零速度指令使小车停下；
 *       关闭窗口时也会自动停车。
...
#include <cmath>                                    // 数学函数，角度转弧度

/**
 * 小车 GUI 控制窗口类
 * 集成 ROS2 节点和 Qt 界面，点击启动持续发送速度，点击停止发送零速度
 */
class VelGuiCtrlWindow : public QWidget
{
public:
    VelGuiCtrlWindow(std::shared_ptr<rclcpp::Node> node, QWidget *parent = nullptr)
...
private:
    /**
     * 初始化界面布局：线速度、角速度输入框，启动和停止按钮
     */
    void init_ui()
    {
        setWindowTitle("小车控制器");       // 窗口标题
        setFixedSize(350, 150);            // 固定窗口大小
        ...
        // ---- 角速度输入行（单位：度/s） ----
        auto *label_angular = new QLabel("角速度 (度)");
        ...
        connect(btn_start, &QPushButton::clicked, this, &VelGuiCtrlWindow::on_start_clicked);
        connect(btn_stop,  &QPushButton::clicked, this, &VelGuiCtrlWindow::on_stop_clicked);
    ...
    /**
     * 初始化定时器：
     * - ros_timer_：每 50ms 处理一次 ROS2 回调
     * - pub_timer_：每 33ms（约30Hz）发布一次速度指令（启动后才开始）
     */
    void init_timers()
    {
        // ROS2 回调定时器，保证 ROS2 和 Qt 事件循环共存
        ...
        connect(pub_timer_, &QTimer::timeout, this, &VelGuiCtrlWindow::publish_velocity);
    }
    ...
    void on_start_clicked()
    {
        ...
        linear_x_   = input_linear_->text().toDouble(&ok_linear);       // 获取线速度
        angular_deg_ = input_angular_->text().toDouble(&ok_angular);     // 获取角速度（度/s）
    ...
    {
        pub_timer_->stop();         // 停止发布定时器
        is_running_ = false;
        publish_stop();             // 发送零速度停车
...
int main(int argc, char **argv)
{
    ...
    auto node = rclcpp::Node::make_shared("vel_gui_ctrl_node");     // 创建 ROS2 节点

    QApplication app(argc, argv);                                   // 创建 Qt 应用
    VelGuiCtrlWindow window(node);                                  // 创建 GUI 窗口
    window.show();                                                  // 显示窗口

    RCLCPP_INFO(node->get_logger(), "vel_gui_ctrl_node 已启动");
